...
def main():
    ...
    raw_url = "https://web.archive.org/web/20180406083923id_/http://kagamine-no-ochakai.jp/circlelist.html"
    ...
    for table in tables:
        ...
        for row in table_rows:
                ...
                if "配置" in position or not position:
                    continue  # Skip header or empty rows

                circle_name = sanitize_string(cols[1].get_text())
                pen_name = sanitize_string(cols[2].get_text())
                circle_urls: list[str] = []
                twitter_hp_tag = cols[3].select_one("a")
                if twitter_hp_tag and twitter_hp_tag.has_attr("href"):
                    circle_urls.append(twitter_hp_tag["href"])
                ...
                url_hp_tag = cols[6].select_one("a")
                if url_hp_tag and url_hp_tag.has_attr("href"):
                    circle_urls.append(url_hp_tag["href"])

                # spaces = sanitize_string(cols[7].get_text())
                description = sanitize_string(cols[8].get_text())
                ...
                circle = Circle(
                    ...
                    links=circle_urls,
                    position=position,
                    comments=description if description else None,
                )
